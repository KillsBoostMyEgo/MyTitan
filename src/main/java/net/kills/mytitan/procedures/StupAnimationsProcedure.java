...
public class StupAnimationsProcedure {
	...
	public static record MyTitanModAnimationMessage(Component animation, int target, boolean override) implements CustomPacketPayload {
		...
		public static void handleData(final MyTitanModAnimationMessage message, final PlayPayloadContext context) {
				...
				context.workHandler().submitAsync(() -> {
					...
					if (level.getEntity(message.target) != null) {
						...
						if (player instanceof AbstractClientPlayer player_) {
							var animation = (ModifierLayer<IAnimation>) PlayerAnimationAccess.getPlayerAssociatedData(player_).get(new ResourceLocation("my_titan", "player_animation"));
							if (animation != null && (message.override ? true : !animation.isActive())) {
								animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation("my_titan", message.animation.getString()))));
							}
						}
					}
				}).exceptionally(e -> {
		...
		public static void registerMessage(FMLCommonSetupEvent event) {
			MyTitanMod.addNetworkMessage(MyTitanModAnimationMessage.ID, MyTitanModAnimationMessage::new, MyTitanModAnimationMessage::handleData);
		}
	}
